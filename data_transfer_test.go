...
func TestPasswordRegistrationRequest_ValidateAndParse(t *testing.T) {
	...
	testutils.ExpectErrorWithMessage(err, "invalid username", t)
	req.Username = "ValidUser"
	req, err = req.ValidateAndParse()
	testutils.ExpectErrorWithMessage(err, "invalid email", t)
...
func TestAddItemRequest_ValidateAndParse(t *testing.T) {
	...
	req, err := req.ValidateAndParse()
	testutils.ExpectErrorWithMessage(err, "name to long", t)
	req.Name = "not too long"
	req, err = req.ValidateAndParse()
	testutils.ExpectError(err, t)
	req.Image = "AAAA"
	req, err = req.ValidateAndParse()
	testutils.ExpectErrorWithMessage(err, "amount must not be negative", t)
	req.Amount = 1
	req, err = req.ValidateAndParse()
	testutils.FailOnError(err, t)
}
...
func TestUpdateItemRequest_ValidateAndParse(t *testing.T) {
	...
	t.Run("name can't exceed 64 chars", func(t *testing.T) {
		...
		req, err := req.ValidateAndParse()
		testutils.ExpectErrorWithMessage(err, "name too long", t)
	})
...
func TestBuyItemRequest_ValidateAndParse(t *testing.T) {
	...
	req, err = req.ValidateAndParse()
	testutils.ExpectErrorWithMessage(err, "amount must be at least one item", t)
...
func TestAddAuthMethodRequest_ValidateAndParse(t *testing.T) {
	...
	req.Method = "nfc"
	req, err = req.ValidateAndParse()
	testutils.ExpectErrorWithMessage(err, "missing nfc uid", t)
	...
	req.Method = "password"
	req, err = req.ValidateAndParse()
	testutils.ExpectErrorWithMessage(err, "invalid method", t)
}
